<?php
    ...
    $result = $student->get_enrld_term();
    $num = $result->rowCount();
    if ($num > 0) {
        //There are enrolled terms for this student
        $result_arr = array();

        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
            extract($row);
            //row contents extracted to variables $year, $season
            $item = array('year' => $year, 
                'season' => $season
            );
        ...
        print_r(json_encode(
            array('message'=>'No Enrolled terms for this student found')
        ));
    }